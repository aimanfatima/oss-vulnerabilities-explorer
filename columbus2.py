    ...
    def report_findings(self, findings: list = []):
        ...
        for i, finding in enumerate(findings):
            ...
            if os.path.exists(f"findings/columbus/{vulnerability_type}/{file_name}"):
                print(f"Skipping existing report: {file_name}")
                continue

            # Create a new file for the report
            with open(f"findings/columbus/{vulnerability_type}/{file_name}", 'w') as f:
            ...
            print(f"Report written for {vulnerability_type}: {file_name}")

        print(f"\nCompleted reporting {len(findings)} findings.")
